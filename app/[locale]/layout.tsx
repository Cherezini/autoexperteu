...
export default function LocaleLayout({
  ...
  const [germanyOpen, setGermanyOpen]     = useState(false); // mobile accordion
  const [germanyHover, setGermanyHover]   = useState(false); // desktop hover
  ...
  // Desktop hover helpers — small delay so dropdown doesn't flicker
      ...
      >
        <div className="container mx-auto px-4">
          <header className="flex items-center justify-between h-[80px] lg:h-[110px]">
                            ...
                            <div className="py-2 px-1">
                              {germanyDropdown.map((sub) => {
                                ...
                                return (
                                  ...
                                  >
                                    <span className="w-1.5 h-1.5 rounded-full bg-[#E6D3A1] flex-shrink-0 opacity-60" />
                                    {sub.name}
                                  </Link>
                                );
                              })}
            ...
            {/* ЯЗЫКИ (десктоп) */}
            <div
              className="hidden lg:flex items-center gap-4 xl:gap-6 text-sm font-black mr-4 lg:mr-6 xl:mr-10"
              style={{ color: '#E6D3A1' }}
            >
              {['es', 'en', 'ru'].map((lang) => (
                <Link
